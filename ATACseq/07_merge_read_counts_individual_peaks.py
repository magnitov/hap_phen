...
def get_counts_table(peaks, sample, replicate, counts_path):
    counts_hap1 = pd.read_csv(f'{counts_path}/counts_{sample}_{replicate}_hap1.txt', 
                              sep = '\t', header = None, names = ['chrom', 'start', 'end', 'peak_id', f'{sample}_{replicate}_hap1'])
    counts_hap2 = pd.read_csv(f'{counts_path}/counts_{sample}_{replicate}_hap2.txt', 
...
for sample in ['NA12878', 'NA18983', 'HG01241', 'HG02601', 'HG03464']:
    # Read peaks
    peaks = pd.read_csv(peaks_path + '/' + sample + '_peaks.canonical.replicated.no_blacklist.bed',
    ...
    counts_allelic.drop(counts_allelic.columns[[0, 1, 2]], axis = 1).to_csv(f'{counts_path}/counts_allelic_{sample}.txt', sep = '\t', header = 1, index = 0)
    ...
    counts_unassigned.drop(counts_unassigned.columns[[0, 1, 2]], axis = 1).to_csv(f'{counts_path}/counts_unassigned_{sample}.txt', sep = '\t', header = 1, index = 0)
