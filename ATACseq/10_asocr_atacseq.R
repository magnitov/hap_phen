...
counts_allelic <- as.matrix(read.csv(sprintf("/DATA/users/m.magnitov/hap_phen/ATACseq/counts/counts_allelic_%s.txt", sample), sep = "\t", row.names = 1))
counts_total <- as.matrix(read.csv(sprintf("/DATA/users/m.magnitov/hap_phen/ATACseq/counts/counts_total_%s.txt", sample), sep = "\t", row.names = 1))

samples_allelic <- read.csv(sprintf("/DATA/users/m.magnitov/hap_phen/ATACseq/asocr/samples_%s_allelic.txt", sample), sep = "\t", row.names = 1)
samples_allelic$replicate <- factor(samples_allelic$replicate)
samples_allelic$allele <- factor(samples_allelic$allele)

samples_total <- read.csv(sprintf("/DATA/users/m.magnitov/hap_phen/ATACseq/asocr/samples_%s.txt", sample), sep = "\t", row.names = 1)
...
dds_allelic <- DESeqDataSetFromMatrix(countData = counts_allelic,
                                      ...
                                      design = ~ allele)
keep <- rowSums(counts(dds_allelic) >= 5) >= 3
...
plotPCA(vsd, intgroup=c("replicate", "allele")) +
  ...
  ggtitle(sprintf("PCA of haplotype counts for %s ATAC-seq samples", sample))
...
sizeFactors(dds_allelic) <- sizeFactors(dds_total)[c(1, 1, 2, 2, 3, 3)]
...
res <- results(dds_allelic, alpha = 0.05)
summary(res)

hist(res$pvalue, main = sprintf("Histogram of p-values for %s ATAC-seq", sample))

res <- res[!is.na(res$padj), ]
write.table(as.data.frame(res), file = sprintf("/DATA/users/m.magnitov/hap_phen/ATACseq/asocr/asocr_%s.csv", sample))
